      ...
      <table class="rank-table">
        <thead>
          <tr>
            ...
            <th class="rk-hide-mob">Prom.</th>
            <th class="rk-hide-mob" title="Resultados exactos en grupos (5 pts)">Exactos</th>
            <th class="rk-hide-mob" title="Exactos con bonus en grupos (7 pts)">Ex+B</th>
            <th>Ult. 5</th>
            <th>Forma</th>
          </tr>
        </thead>
...
function renderChart(data){
  ...
  var datasets=data.players.map(function(name){return{label:name,data:data.history[name],borderColor:data.colors[name]||'#fff',backgroundColor:data.colors[name]||'#fff',tension:0.2,borderWidth:1.5,pointRadius:0,pointHoverRadius:4};});
  ...
  chartInstance=new Chart(ctx2d,{type:'line',data:{labels:labels,datasets:datasets},options:{responsive:true,maintainAspectRatio:false,layout:{padding:{right:340}},interaction:{mode:'index',intersect:false},scales:{y:{beginAtZero:true,grid:{color:'rgba(255,255,255,.07)'},ticks:{color:'#7d8590'}},x:{grid:{display:false},ticks:{color:'#7d8590',maxRotation:90,minRotation:90,font:{size:9},autoSkip:false}}},plugins:{legend:{display:false},tooltip:{mode:'index',intersect:false,itemSort:function(a,b){return b.raw-a.raw;},backgroundColor:'rgba(13,17,23,.97)',titleColor:'#00e5ff',bodyColor:'#ccc',borderColor:'#30363d',borderWidth:1,padding:10,callbacks:{label:function(){return null;},beforeBody:function(items){return items.map(function(it,i){return'  '+(i+1)+'. '+it.dataset.label+': '+it.raw;});}}},zoom:{pan:{enabled:true,mode:'x'},zoom:{wheel:{enabled:true,speed:0.08},pinch:{enabled:true},mode:'x'}}}},plugins:[dimPlugin,drawNamesPlugin]});
